//delete modal// src/features/dashboard/leads/components/DeleteModal.tsx
...
export function DeleteModal({ isOpen, onClose, onConfirm, leadName, isLoading = false }: DeleteModalProps) {
  return (
    <Modal
      backdrop='opaque'
      classNames={{
            ...
            <ModalHeader className='flex flex-col gap-0 border-gray-100 border-b pb-4'>
              <div className='flex w-full items-center justify-between'>
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    <h3 className='font-semibold text-gray-900 text-lg'>تأیید حذف سرنخ</h3>
                ...
                <div className='rounded-lg border border-amber-100 bg-amber-50 p-4'>
                  <p className='text-gray-700 text-sm leading-relaxed'>
                    {leadName ? (
                      <>
                        آیا از حذف سرنخ{" "}
                        <span className='rounded bg-amber-100 px-2 py-0.5 font-bold text-primary'>{leadName}</span>{" "}
                        اطمینان کامل دارید؟
                      </>
                    ) : (
                      "آیا از حذف این سرنخ اطمینان کامل دارید؟"
                    )}
                  </p>
                </div>
              </div>
