...
export function AccountsToolbar() {
  ...
  const handleFileUpload = async (file: File) => {
    setIsUploading(true);
    try {
      const result = await companyService.importExcel(file);
      showToast.success(result.message);
      if (result.failed && result.failed > 0) {
        showToast.info(`افزودن سازمان با خطا مواجه شد`);
      }
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      showToast.success(
        selectedIds.length === 1 ? "سازمان با موفقیت حذف شد" : `${selectedIds.length} سازمان با موفقیت حذف شدند`,
      );
      setSelectedIds([]);
