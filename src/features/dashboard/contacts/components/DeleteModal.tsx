...
export function DeleteModal({ isOpen, onClose, onConfirm, contactName, isLoading = false }: DeleteModalProps) {
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
          ...
          <>
            <ModalHeader className='flex flex-col gap-0 border-b border-gray-100 pb-4'>
              <div className='flex items-center gap-3'>
                ...
                <div>
                  <h3 className='font-semibold text-gray-900 text-lg'>تأیید حذف مخاطب</h3>
            ...
            <ModalBody>
              <div className='rounded-lg border border-amber-100 bg-amber-50 p-4 text-sm text-gray-700'>
                {contactName ? (
                  <>
                    آیا از حذف مخاطب <span className='font-bold text-primary'>{contactName}</span> مطمئن هستید؟
                  </>
                ) : (
