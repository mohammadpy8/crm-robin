...
interface DeleteModalProps {
  ...
  isLoading?: boolean;
  entityName?: string;
  entityType: "مخاطب" | "سرنخ" | "کاربر" | "سازمان" | string;
}
            ...
            <ModalHeader className='flex flex-col gap-0 border-gray-100 border-b pb-4'>
              <div className='flex w-full items-center justify-between'>
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    <h3 className='font-semibold text-gray-900 text-lg'>تأیید حذف {entityType}</h3>
              ...
              <div className='space-y-4'>
                <div className='rounded-lg border border-amber-100 bg-amber-50 p-4'>
                  <p className='text-gray-700 text-sm leading-relaxed'>
                    {entityName ? (
                      <>
                        آیا از حذف {entityType}{" "}
                        <span className='rounded bg-amber-100 px-2 py-0.5 font-bold text-primary'>{entityName}</span>{" "}
                        اطمینان کامل دارید؟
                      </>
                    ) : (
                      `آیا از حذف این ${entityType} اطمینان کامل دارید؟`
                    )}
                  </p>
                </div>
              </div>
